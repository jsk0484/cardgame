...
**7-1. lastPlay 카드 앞면이 공개되어 블러핑 의미가 훼손됨** ← 핵심 게임성 문제

현재 `PlayArea.tsx` 는 `lastPlay.cards` 를 `<CardComponent ... />` 로 앞면 그대로 렌더링하고 있음.

```tsx
// PlayArea.tsx:23 — 현재 (문제)
{lastPlay.cards.map(card => (
  <CardComponent key={card.id} card={card} small />  // 앞면 공개
))}
```

| 표시 정보 | 기획 의도 | 현재 구현 |
|----------|----------|----------|
| 선언한 조합 타입 | ✅ 공개 (도전 판단 근거) | ✅ 공개 |
| 낸 카드 수 | ❌ 숨김 (장수도 힌트가 됨) | ✅ 공개 중 |
| 카드 뒷면 | ❌ 숨김 (장수 노출 방지) | ❌ **공개 중 (버그)** |
| 실제 카드 앞면 | ❌ 숨김 (블러핑 보호) | ❌ **공개 중 (버그)** |

- 카드 장수도 조합 판단의 힌트가 됨 (예: 3장 뒷면 → "트리플 의심 가능")
- **선언한 조합 타입 텍스트만** 표시하는 것이 올바른 블러핑 설계
- 도전 성공/실패 이후에만 실제 카드 앞면 공개

→ `components/PlayArea.tsx` 수정 필요: 카드 컴포넌트 전체 제거, 선언 타입 + 누가 냈는지 텍스트만 표시
...
| 우선순위 | 항목 | 위치 |
|---------|------|------|
| 🔴 높음 | lastPlay 카드(앞면/뒷면/장수) 모두 숨김 → 선언 타입 텍스트만 표시, 도전 결과 시 앞면 공개 | `PlayArea.tsx` |
