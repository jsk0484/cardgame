...
**2-1. 라운드 종료 점수 계산 방식이 다름**

| | 기획 | 실제 구현 |
|-|------|----------|
| 손패 합산 | 남은 손패 카드 값 합산 점수 부여 | 남은 손패 카드 값을 **패널티**로 차감 (`-floor(val/5)`) |
| 플러시/스트레이트/트리플 보너스 | 라운드 종료 시 적용 | `calcRoundScore()` 함수는 존재하나 `_endRound()`에서 **호출되지 않음** |
| 핸드아웃 보너스 | +10 | +10 (동일) |

→ `gameStore.ts: _endRound()` 에서 `scoring.ts: calcRoundScore()` 미사용

**2-2. 플레이어 수가 2인으로 고정됨**

| | 기획 | 실제 구현 |
|-|------|----------|
| 인원 | 2~4인 | 플레이어 1 + AI 1 (하드코딩) |

→ `gameStore.ts: HUMAN_IDX = 0, AI_IDX = 1`, `dealHands(deck, 2, ...)`

**2-3. 게임 화면이 라우트가 아닌 상태 전환 방식으로 구현됨**

| | 기획 | 실제 구현 |
|-|------|----------|
| 화면 전환 | `/`, `/lobby`, `/room/:id`, `/game/:id`, `/result/:id` 라우트 | `GameScreen = 'main' \| 'game' \| 'result'` 상태값으로 전환 |
| 로비 화면 | 별도 존재 | 없음 |

→ `types.ts: GameScreen`, `App.tsx`

**2-4. AI 봇이 단일 난이도로 구현됨**

| | 기획 | 실제 구현 |
|-|------|----------|
| AI 난이도 | Open Issue (Easy/Normal/Hard 미정) | 단일 난이도, 도전 확률 20% 고정, 패스 확률 25% 고정 |
| 블러핑 | 언급 | 20% 확률로 선언 타입 변경 |

→ `ai.ts: aiShouldChallenge(), aiChoosePlay()`

**2-5. `_endRound()` 에서 드로우 파일 소진 시 버린 파일 재셔플 처리됨**

| | 기획 | 실제 구현 |
|-|------|----------|
| 드로우 파일 소진 | 즉시 라운드 종료 | 버린 파일을 셔플해서 드로우 파일로 재활용 후 계속 진행 |
...
- 관련 기획 항목: 이벤트 명세 전체, Redis 키 구조, 방 매칭 흐름
...
- 현재: 덱에 카드는 존재하나 사용 시 일반 카드처럼 처리됨 (효과 없음)
→ `deck.ts` 에 카드 존재, `gameStore.ts`에 특수 처리 로직 없음

**3-4. 조커 효과 미구현**
- 기획:
  - `red_joker`: suit + rank 자유 선언
  - `black_joker`: 상대 손패 1장 랜덤 탈취
- 현재: 덱에 카드는 존재하나 일반 카드처럼 처리됨
→ `deck.ts` 에 카드 존재, 효과 처리 로직 없음
...
- 현재: 순수 프론트엔드 (Vite + React)만 존재, 서버 없음

**3-7. 모바일 반응형 미완성 (M5 미착수)**
- 기획: 모바일 반응형 + PWA 검토
- 현재: 미착수
...
**4-3. 드로우 파일 소진 시 버린 파일 자동 재셔플**
- 기획에는 소진 즉시 라운드 종료로 명시되어 있으나, 게임 끊김 방지를 위해 재셔플 로직 추가됨
...
| 마일스톤 | 기획 목표 | 상태 |
|---------|----------|------|
| M1 | 카드 데이터 모델 + 덱 셔플/배분 + 점수 계산 함수 | ✅ 완료 (단, 점수 함수 일부 미연동) |
| M2 | Socket.io 서버 + 룸/턴 관리 | ❌ 미착수 |
| M3 | React 게임 화면 + 카드 렌더링 + Socket 연동 | 🟡 화면/렌더링만 완료, Socket 없음 |
| M4 | 도전/블러핑 + 특수 카드 + 타이머 + 재접속 | 🟡 도전/타이머 완료, 특수 카드 미구현 |
| M5 | 매칭 + 초대 + 모바일 + 배포 | ❌ 미착수 |
...
| 우선순위 | 항목 | 파일 |
|---------|------|------|
| 높음 | `_endRound()`에서 `calcRoundScore()` 연동 | `gameStore.ts` |
